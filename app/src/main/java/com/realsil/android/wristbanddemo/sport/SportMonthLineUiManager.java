...
public class SportMonthLineUiManager {
    ...
    public SportMonthLineUiManager(List<SportData> sports, int year, int month, int day) {
        ...
        for(int i = 1; i <= mMaxDayInMonth; ++i) {
            SportSubData sumOfDay = new SportSubData();
            // sum every day in the month
            SportSubData subData = WristbandCalculator.sumOfSportDataByDate(year
                    , month
                    , i
                    , sports);
            if (subData != null) {
            ...
            mMonthSportDataMap.put(i, sumOfDay);
        }
    }
    public SportSubData getLinePointDetailData(int day) {
        return mMonthSportDataMap.get(day);
    }
    ...
    public LineChartData getSportStepLineData() {
        ...
        for (int i = 1; i <= mMaxDayInMonth; ++i) {
            if(mMonthSportDataMap.get(i) == null) {
                ...
                values.add(new PointValue(i, mMonthSportDataMap.get(i).getStepCount()));
            }
        }
