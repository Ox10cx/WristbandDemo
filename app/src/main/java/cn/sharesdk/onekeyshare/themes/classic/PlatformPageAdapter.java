...
public abstract class PlatformPageAdapter extends ViewPagerAdapter implements OnClickListener {
	...
	private View createPanel(Context context) {
		...
		for (int i = 0; i < lineCount; i++) {
			...
			lp.leftMargin = DensityUtils.dip2px(context, 16);
			lp.rightMargin = DensityUtils.dip2px(context, 16);
//			lp.setMargins(DensityUtils.dip2px(context, 16),DensityUtils.dip2px(context, 16),DensityUtils.dip2px(context, 16),DensityUtils.dip2px(context, 16));
...
//
//			View vSep = new View(context);
//			vSep.setBackgroundColor(Color.parseColor("#898B8E"));
...
//			lp = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, logoHeight);
			...
			tvName.setTextColor(0xffbfbfbf);
			...
			lp = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
			lp.weight = 1;
			lp.topMargin = DensityUtils.dip2px(context, 7);
